...
.ToList();
            ...
            foreach (var item in data2)
            {
                ...
                {
                    if (Core.Program.Callerlocalizer[item.Name].ResourceNotFound == true)
                    {
                        item.Name = Core.Program._localizer[item.Name];
                    }
                    else
                    {
                        item.Name = Core.Program.Callerlocalizer[item.Name];
                    }
                }
                ...
                {
                    if (Core.Program.Callerlocalizer[act.Text].ResourceNotFound == true)
                    {
                        act.Text = Core.Program._localizer[act.Text];
                    }
                    else
                    {
                        act.Text = Core.Program.Callerlocalizer[act.Text];
                    }
                }
